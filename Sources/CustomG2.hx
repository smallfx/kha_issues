...
class ImageShaderPainter {
	...
	static function initShaders(): Void {
		if (structure == null) {
			structure = CustomG2.createImageVertexStructure();
		}
		if (standardImagePipeline == null) {
			var pipeline = CustomG2.createImagePipeline(structure);
...
class ColoredShaderPainter {
	...
	static function initShaders(): Void {
		if (structure == null) {
			structure = CustomG2.createColoredVertexStructure();
		}
		if (standardColorPipeline == null) {
			var pipeline = CustomG2.createColoredPipeline(structure);
...
class TextShaderPainter {
	...
	static function initShaders(): Void {
		if (structure == null) {
			structure = CustomG2.createTextVertexStructure();
		}
		if (standardTextPipeline == null) {
			var pipeline = CustomG2.createTextPipeline(structure);
...
class CustomG2 extends kha.graphics2.Graphics {
	var myColor: Color;
	var myFont: Font;
	var projectionMatrix: FastMatrix4;
	...
	static var current: CustomG2 = null;
  ...
  public function useCanvas(newCanvas: Canvas) {
